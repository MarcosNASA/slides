...
import { PATH_COMPOUND_TESTING, PATH_REACT_PATTERNS } from '../constants'
...
export const CompoundTesting = () => {
  const { nextSlide, previousSlide } = useSlides({
    path: PATH_COMPOUND_TESTING,
    numberOfSlides: compoundTestingSlides.length,
  })
  ...
  return (
    <>
      ...
      <SlideSection>
        <Routes>
          {compoundTestingSlides.map((slide) => (
            <Route
              key={slide.id}
              path={`/slide/${slide.id}`}
